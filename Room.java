...
public class Room 
{
	...
	public boolean takeExit(String exitName)
	{
		for(int i = 0; i < this.theExits.length; i++)
		{
			...
			{
				if(this.id == Exit.getDestinationID())
				{
				this.id = Exit.getDestinationID()+1;
				CaveCore.theRooms[this.id].addThePlayer(thePlayer);
				CaveCore.theRooms[i].removeThePlayer();
				}
			}
			System.out.println("the " + this.thePlayer +" has been added to room " + this.name);
